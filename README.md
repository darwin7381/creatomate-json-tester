...
本工具提供了在線預覽Creatomate JSON腳本的功能，方便測試和調整視頻配置。

### 功能特點

- **實時預覽**: 輸入JSON腳本，即時查看視頻效果
- **範例載入**: 內置基本視頻模板，一鍵載入
- **檔案上傳**: 支持從本地上傳JSON檔案
- **瀏覽JSON**: 可以瀏覽並加載項目中已有的JSON檔案
- **複製API格式**: 一鍵轉換為Creatomate API所需的JSON格式並複製到剪貼板
...
2. **訪問預覽頁面**:
   在瀏覽器中打開 http://localhost:3000/preview （如果3000端口被占用，可能會使用3001或其他端口）

3. **輸入或載入JSON**:
   - 在左側文本框中輸入Creatomate兼容的JSON腳本
   - 點擊「載入示例」按鈕使用預設範例
   - 點擊「從文件載入」上傳本地JSON文件
   - 點擊「瀏覽所有JSON」從項目中選擇已有的JSON文件

4. **查看預覽**:
   右側窗口會實時顯示JSON腳本生成的視頻效果

5. **複製API格式**:
   - 點擊「複製 API Json」按鈕將當前JSON轉換為API格式並複製到剪貼板
   - 轉換後的格式為 `{ "source": 原始JSON }`，可直接用於API請求

### 注意事項

- 確保JSON格式正確，否則預覽可能失敗
- 大型或複雜的視頻預覽可能需要較長載入時間
- 修改JSON後，預覽會自動更新，無需手動刷新
- 成功操作（如複製API格式）會顯示綠色成功提示
- 出錯時會顯示紅色錯誤提示，提供排錯信息
